...
def get_char_freq(data_file, char_freq_file='char_freq_table',is_dict=False):

    data=data_file.read()
    
    char_freq_table=defaultdict()
    for char in data:
        if char not in char_freq_table.keys():
            char_freq_table[char]=data.count(char)
    #normalize
    max_charfreq_in_table=1
    for char in char_freq_table.keys():
        char_freq_table[char] = math.sqrt(char_freq_table[char]) #reduce total size, float is limited
        max_charfreq_in_table=max(max_charfreq_in_table, char_freq_table[char])

    for char in char_freq_table.keys():
        char_freq_table[char]= (char_freq_table[char]/max_charfreq_in_table) # normalized(0,1) in ivocab, vocab is divided with char_freq, will be larger.

    return char_freq_table

def get_ivocab_from_vocab(vocab, char_freq_table, char_freq_file='char_freq_table',is_dict=False,outfile='ivocab',load_char_from_file=True):

    if load_char_from_file:
        char_freq_table=load_pkl(char_freq_file)
    ivocab=copy.deepcopy(vocab)
    pattern = re.compile(r'[0-9A-Za-z]+', re.I)
    for key in vocab.keys():

        discard = pattern.search(key)
        if discard !=None :
            ivocab.pop(key)
            continue

    for key in ivocab.keys():
        if len(key)>1:
            max_char_freq_in_word=0.2
            for char in key:
                #print(char,end='')
                max_char_freq_in_word=max(max_char_freq_in_word, int(char_freq_table[char]))
            #print('next key')
        else: 
            max_char_freq_in_word = char_freq_table[char]
        
        ivocab[key]=vocab[key]/(max_char_freq_in_word*5)
    #print('vocab.keys done')
    if outfile!= None:
        save_pkl(ivocab,outfile)
...
def learn_bpe(infile, outfile, num_symbols, min_frequency=2, verbose=False, is_dict=False, total_symbols=False, num_workers=1):
    ...
    if not os.path.exists('vocab.pkl'):
        ...
        save_pkl(vocab,'vocab')
    else:
        vocab=load_pkl('vocab')

    #vocab = get_vocabulary(infile, is_dict, num_workers)

    if not os.path.exists('char_freq_table.pkl'):
        char_freq_table=get_char_freq(infile)#mod
        save_pkl(char_freq_table,'char_freq_table')
    else:
        char_freq_table=load_pkl('char_freq_table')

    ivocab=get_ivocab_from_vocab(vocab, char_freq_table, load_char_from_file=False)#mod#default will save ivocab
    save_pkl(ivocab, 'ivocab')
    vocab=ivocab   #
    
    vocab = dict([(tuple(x[:-1])+(x[-1]+'</w>',) ,y) for (x,y) in vocab.items()])

    sorted_vocab = sorted(vocab.items(), key=lambda x: x[1], reverse=True)
    save_pkl(sorted_vocab, 'sort_ivocab')
    ...
    #print('stats.values()',stats.values())
...
#     vocab = get_vocabulary(infile, is_dict, num_workers)
#     get_char_freq(infile)#mod
#     ivocab=get_ivocab_from_vocab(vocab)#mod
#     vocab=ivocab#mod
